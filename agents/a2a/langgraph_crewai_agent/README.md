...
Uses **[Helm](https://helm.sh/)** + **`Makefile`**. Chart: **`infrastructure/helm/a2a-langgraph-crewai/`** (two Deployments, two Services, two Routes, one Secret).
...
| Resource | Role |
|----------|------|
| Helm chart **`infrastructure/helm/a2a-langgraph-crewai`** | Secret, Services, Routes, Deployments |
